...
resolution: "readPairRequest now reads one byte past maxPairBodyBytes and refuses a body that reaches it with a 413, and unmarshals the whole body so trailing content inside the bound is refused too; TestThePairingEndpointRefusesABodyOverItsLimit drives the endpoint end to end and holds that nothing is written."
impact: fix
---
...
- pursued: we expect a bounded unauthenticated endpoint to refuse an oversized body outright because a bound that only stops the reading accepts whatever prefix parsed; wrong if a real client legitimately sends more than 4 KiB to /pair, which a name and a P-256 SPKI cannot.
