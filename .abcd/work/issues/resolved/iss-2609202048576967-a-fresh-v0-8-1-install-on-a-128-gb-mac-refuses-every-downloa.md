...
resolution: "Fixed on fix/default-served-window: the default served window is derived — the largest window that fits the memory budget at the batched requests in force, capped at the declared window, floored at 4096 — so a fresh install serves every model it holds; the default batched requests is one so the window is as wide as the budget allows; the warning names served context and batched requests, not the budget. Hand-checked on the built binary against a small model on a scratch root: at a 1 GiB budget the list read served_context 11820 (default), a request inside it was served and one of 20034 tokens refused naming the 11820; at 77 GiB the window read the declared 32768."
impact: fix
resolved_by:
  commit: "5eda2a07"
---
...
- pursued: we expect a window derived from the budget to make every downloaded model servable on a fresh install without a setting, and it is wrong if a model that fit before this change is refused after it, or if a model is ever charged less than it costs
