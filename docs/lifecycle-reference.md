...
The Dessau binary answers to a handful of verbs typed in a terminal: what this
...
A verb is reached through the `dessau` command, which the install links into
`~/.local/bin`. The same binary is inside the application bundle, at
`DessauServer.app/Contents/MacOS/dessau`, and answers the same verbs when it is run
by its full path.
...
| --- | --- | --- |
| `dessau` | With no arguments at all, starts the server. This is how macOS launches the bundle, and it does not change. | the server's own |
| `dessau serve` | The bare invocation written out. Flags after it are the server's. | the server's own |
| `dessau version` | Prints which build this is, read from the binary. Contacts nothing. | `0` |
| `dessau status` | What this installation is doing right now. Reads state that already exists, so polling it is safe. | `0` |
| `dessau doctor` | The expensive checks, each labelled with how much its answer is worth. | `0` or `1` |
| `dessau config show` | The settings in force, read from the settings file the way the server reads it. Reads only; the API key and the HuggingFace token are shown as `********` and never as their values. | `0` |
| `dessau install` | Places the application, grants it through the firewall, provisions the MLX runtime, links the command. Repairs what is missing. | `0` or `1` |
| `dessau place` | Puts an application bundle into a directory with the staged swap: stages inside the destination under an unguessable name, sets the installed bundle aside, renames the new one in, and removes the set-aside copy only once the new one is there. Refuses a destination directory that is a symbolic link, replaces rather than follows a symbolic link at the bundle's own name, and never nests inside a bundle that is already there. The bootstrap calls it to place the chat client, which carries no binary of its own. | `0` or `1` |
| `dessau uninstall` | Removes what this installation put on the Mac. Leaves the downloaded models. | `0` or `1` |
| `dessau update` | Fetches the current release, verifies it against the checksums published beside it, places it with the staged swap, re-makes the firewall grant, and reports the version installed and the version this Mac is serving as two separate facts. | `0` or `1` |
...
`dessau -headless` and `dessau -root …` keep their meaning.

## The flags

| Verb | Flag | What it does |
| --- | --- | --- |
| `dessau status` | `--json` | Writes the machine-readable form on standard output. That form is the contract. |
| `dessau doctor` | `--json` | Writes the machine-readable form, which carries the severity of every finding. |
| `dessau config` | `--json` | Writes the machine-readable form, which is the contract: an object of settings keyed by the name `config.json` gives each one. |
| `dessau install` | `--bundle` | Takes a path: the verified bundle to place. The bootstrap passes it; a person repairing an installation does not. |
| `dessau install` | `--place-only` | Places the bundle and stops: no firewall grant, no provisioning, no launch. This is what the release gate runs, where there is no console to answer an authorisation panel. |
| `dessau place` | `--bundle` | Takes a path: the application bundle to place. |
| `dessau place` | `--into` | Takes a path: the directory the bundle belongs in. It must already be there, and it must be a directory rather than a symbolic link. |
| `dessau uninstall` | `--purge` | Removes the downloaded models as well. |
| `dessau uninstall` | `--yes` | Answers the confirmation `--purge` would otherwise need a terminal for. |
...
| `1` | The verb ran and something Dessau verified is wrong, or a stage of the install failed. |
...
by a process that could not prove it is this account's Dessau — which refuses
...
| `serving` | boolean | Whether this account's Dessau answered the instance challenge on the port. |
...
| --- | --- | --- |
| `verified` | State Dessau owns and read for itself. A severity here means what it says. | `ok`, `warning` or `failed` |
| `observed` | A query about state Dessau does not own answered, and what it answered could not be established. The line reports what came back and is **never a verdict**, in either direction. | `undetermined`, always |
...
| server port | `verified` | Who holds the port: this account's Dessau, one process that is not it, or nothing. A foreign holder is counted and not named, and the line says the build reported above is this binary's own. |
...
The install creates a symbolic link at `~/.local/bin/dessau`, pointing at the
...
Uninstall removes that link only when it is a symbolic link into a Dessau
bundle. A real file of that name belongs to whoever put it there.

## `dessau config show --json`
...
belongs to. Every setting Dessau holds is in it, including the ones that have
...
- **No deletion path comes from the environment or the command line.**
  ...
  `DESSAU_ROOT` is read only so the output can name the root it did not
  remove.
- **Only `dessau update` contacts the network, and only when it is typed.**
  ...
  for a release at launch, and Dessau never learns that a Mac is behind.
- **Nothing else reaches the forge, and the address cannot be moved.** The
  release `update` fetches from is fixed in the binary: nothing Dessau reads —
...
- **No verb writes a setting.** The control panel is the settings file's only
  ...
  server. `dessau config show` reads it and answers.

## Where to go next

- [Install, repair and remove Dessau](lifecycle.md) — the how-to for each of
  these verbs.
