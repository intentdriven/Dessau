...
| **Self-test** | Whether the self-test is on: that while the Mac is idle Gropius loads one of its models at a time where it fits beside what is loaded, measures it with a fixed set of prompts and unloads what it loaded; that any request ends the run; and that the figures go to a file in this account's data folder and hold no prompt and no answer. When it is off, that Gropius loads no model on its own. | The `self_test` setting, which is applied live. |
