...
where the snapshot cannot carry a fact, the line says what Dessau cannot see
...
| **Debug logging** | Present while a model is armed for debug logging or running at it. Which models are armed, to be logged from their next start, and which are on, their model server having been started with it; that while it is on the model server's own log holds every request sent to it and every answer it produced — the prompts and the completions, whoever sent them — and the requests Dessau's own probes and self-test send; that it ends when that model server next starts, which memory pressure or another client's request can cause; that the file is in this account's logs folder, created for this account alone, stops at 64 MB, and that the previous run's file is kept beside it; that nothing is sent anywhere and clients are not told. Absent when no model is armed or at debug. | The armed set, and each resident model's running level. |
...
- **Who reaches an address.** The bind is observable; reachability is not.
  ...
  on, and Dessau reads no network's settings.
- **Every address under the wildcard.** A wildcard bind answers on every
  address this Mac holds. The list names the IPv4 addresses Dessau found and
...
- **What has happened to a private network.** The mark states which network an
  ...
  gives traffic between two machines is the network's doing, and Dessau neither
  performs nor observes it.
- **An announcement that failed to start.** The local-network line is read from
  the decision Dessau made when it started, from the setting and the bind
...
- **A setting saved since the start.** The port, the bind and the `advertise`
  setting are taken once, when Dessau starts. A change saved since then is in
...
Dessau announces this server when all three hold: the `advertise` setting is
