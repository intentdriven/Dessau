...
var notEnforcement = map[string]string{
	...
	"github.com/intentdriven/Dessau/internal/toolprobe":      "asks a served model one question with a tool declared and records whether it called it (itd-2609201445423499). It decides nothing about who may reach the server or what it will do for them: it talks only to the model server the pool hands it, on loopback, and it imports config for the fold rule, registry for the verdict's shape and selftest for the yield its hold carries, nothing else of ours, so there is no address in it to enforce on. The verdict it writes is informative only — nothing on the completions path reads it, which toolprobe_test.go in this directory and the gateway's own test hold",
