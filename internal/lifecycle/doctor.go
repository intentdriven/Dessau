...
func checkFirewall(env DoctorEnv) Answer {
	...
	// by different things. The prose is for a person, so the path is redacted
	// there like every other string in the report. The command is for a
	// shell, so it is rendered by shellArg — the same rule the install and the
	// uninstall print their commands under: quoted against a space or a quote
	// in the path, and written "$HOME/…" rather than "~/…" so that it still
	// carries no account name and still runs.
	binary := redact(env.Binary, env.Home)
	answer := Answer{Commands: firewallGrantCommands(env.Binary, env.Home)}
