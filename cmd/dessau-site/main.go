// Command dessau-site renders the public landing page at intentdriven.sh/Dessau.
//
...
// Usage:
//
//	go run ./cmd/dessau-site --out site                        render the page
//	go run ./cmd/dessau-site --out site --release release.json render it with the release facts
//	go run ./cmd/dessau-site select --from releases.json       print the tag of the release the page names
//	go run ./cmd/dessau-site validate-release release.json     say whether a record is fit to render
//
...
//	  "assets": [
//	    {"name": "DessauServer.app.zip", "size_bytes": 3172806, "url": "https://…"}
//	  ]
//	}
//
...
func main() {
	if err := run(os.Args[1:]); err != nil {
		fmt.Fprintln(os.Stderr, "dessau-site:", err)
		os.Exit(1)
	}
}
...
func run(args []string) error {
	...
	case "validate-release":
		...
		fmt.Printf("dessau-site: %s is a release record this page can render\n", fs.Arg(0))
		return nil
...
func render(root, manifestPath, out, releasePath string) error {
	...
	fmt.Printf("dessau-site: rendered %s\n", filepath.Join(dir, "index.html"))
	return nil
}
