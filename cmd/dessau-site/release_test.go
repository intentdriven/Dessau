...
const goodRecord = `{
  ...
  "assets": [
    {"name": "DessauServer.app.zip", "size_bytes": 20971520, "url": "https://example.invalid/releases/download/v1.2.3/DessauServer.app.zip"},
...
func TestTheRecordIsRefused(t *testing.T) {
	for _, tc := range []struct {
		...
		{"an asset served from another host", patchAsset(t, 0, func(a map[string]any) {
			a["url"] = "https://cdn.evil.invalid/DessauServer.app.zip"
		}), "url"},
		{"an asset from another repository", patchAsset(t, 0, func(a map[string]any) {
			a["url"] = "https://example.invalid/someone-else/releases/download/v1.2.3/DessauServer.app.zip"
		}), "url"},
		{"an asset from another release", patchAsset(t, 0, func(a map[string]any) {
			a["url"] = fixtureRepo + "/releases/download/v0.0.1/DessauServer.app.zip"
		}), "url"},
