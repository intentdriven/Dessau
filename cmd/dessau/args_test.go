...
// left over. `dessau install` therefore bound the configured address,
...
// nothing at all — otherwise `dessau serve -headless` would start a menu bar
...
func TestATrailingWordIsRefusedAfterTheFlags(t *testing.T) {
	...
	if !strings.Contains(msg, "dessau status") {
