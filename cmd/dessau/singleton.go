...
// always called it. The classification itself lives there because `dessau
...
// defer to a process that cannot prove it is this user's Dessau.
//
// A plain net.Listen is not enough. When Dessau is quit and relaunched right
...
//   - holderOurs   → a Dessau sharing our data root; defer to it as a client.
	...
	for {
		...
		case holderForeign:
			return nil, false, fmt.Errorf(
				"port %s is held by a process that is not this user's Dessau; "+
					"refusing to route local model traffic to it (another account may be impersonating the server)", addr)
		default: // holderNone
			if time.Now().After(deadline) {
				return nil, false, fmt.Errorf(
					"port %s is busy but no Dessau server is responding on it", addr)
			}
			time.Sleep(retry)
		}
	}
