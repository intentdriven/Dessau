...
// That rule is older than the verbs and is why this file exists — `dessau
...
// like `dessau doctr --json` leaves both words here because the first word is
...
func refuseUnknownArgs(args []string) string {
	...
	return "dessau: unknown argument " + lifecycle.Quote(args[0]) + "\n" +
		"The verbs this build knows are: " + strings.Join(knownVerbs(), ", ") + ".\n" +
		"Run dessau with no arguments to start the server."
}

// refuseTrailingArgs returns the message to print for a word left over after
// the server's flags were parsed.
//
// Go's flag package stops at the first non-flag argument, so `dessau -headless
...
func refuseTrailingArgs(args []string) string {
	...
	if _, known := verbs[args[0]]; known {
		return "dessau: " + lifecycle.Quote(args[0]) + " is a verb, and a verb comes first: dessau " + args[0] + "\n" +
