// Command dessau is the menu-bar app and model server.
//
...
func main() {
	...
	// `dessau serve`, where the verb itself is not one of the server's flags.
	...
	// the wrong default for this program in particular: `dessau install` bound
	...
	if !claimed {
		log.Info("another Dessau server is already running; starting as a client",
			"port", cfg.Port)
...
// choose them stays inside the resolver, and cmd/dessau acquires what it is
...
// runClient is a secondary instance: another Dessau already owns the port, so
