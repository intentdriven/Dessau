...
func fakeVerbEnv() (lifecycle.Env, error) {
	root, err := os.MkdirTemp("", "dessau-verb-env")
	if err != nil {
...
func TestAWritingVerbIsDispatchedToTheFake(t *testing.T) {
	...
	if _, err := os.Lstat(filepath.Join(home, ".local", "bin", "dessau")); err == nil {
...
// reading verbs: `dessau config show` answers from the environment it is
