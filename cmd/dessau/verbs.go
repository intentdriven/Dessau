...
//
// cmd/dessau's TestMain replaces this, and the writing entries of
...
func runCommandVerb(cmd commandLine, out, errOut io.Writer) int {
	...
	case verbVersion:
		...
		// `dessau version --json` that they had got what they asked for.
		if len(cmd.Args) > 0 {
			fmt.Fprintln(errOut, "dessau version: unexpected argument "+lifecycle.Quote(cmd.Args[0]))
			return lifecycle.ExitUsage
		}
		fmt.Fprintln(out, "dessau "+version)
		return 0
	case verbNotYet:
		// Named rather than dismissed: the person who read the documentation
		// and typed this learns that they typed it correctly.
		fmt.Fprintln(errOut, "dessau "+cmd.Verb+": not in this build yet")
		return lifecycle.ExitUsage
	}
	...
	if !ok {
		...
		fmt.Fprintln(errOut, "dessau "+cmd.Verb+": this build lists the verb and does not run it")
		return lifecycle.ExitUsage
	}
	env, err := verbEnvFor()
	if err != nil {
		fmt.Fprintln(errOut, "dessau "+cmd.Verb+": "+err.Error())
