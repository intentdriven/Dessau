...
//     a foreign process or an older, single-listener Dessau. Loopback is
...
// nil error means another Dessau owns the port and this instance is a client.
func acquireBind(plan bind.Plan, port int, wait time.Duration, holder func() portHolder) ([]net.Listener, bind.Plan, bool, error) {
	...
	for {
		...
		// this: a name is a bind Dessau treats as exposed and generates a key
		...
		default: // holderNone: a predecessor is probably still going down
			if time.Now().After(deadline) {
				return nil, plan, false, fmt.Errorf(
					"port %s is busy but no Dessau server is responding on it", addrs[1])
			}
			time.Sleep(retry)
		}
	}
